...
# Get TPE ------------------------------------------------------------------------
tpe <- strsplit(dataset_file_name, "_")[[1]][2]


# GENERAL FILES PATHS ------------------------------------------------------------
sumary_results_file_path <- paste(results_folder, paste(sumary_results_file, "_Test.csv", sep = ""), sep = "/")
sumary_all_results_file_path <- paste(results_folder, paste(sumary_results_file, "_ALL_Test.csv", sep = ""), sep = "/")
...
if (file.exists(sumary_results_file_path)) {
  general_sumary_results <- read.csv(sumary_results_file_path) %>% select(-X)
  general_sumary_all_results <- read.csv(sumary_all_results_file_path) %>% select(-X)
...
Trait
# INDIA #
Pheno_India=Pheno[Pheno$TPE==tpe,]
...
# LOCATIONS INDIA #
india_envs <- unique(Pheno_India$Env)
# GENOMA #
  ...
  for (m in 1:length(models_list)) {
    ...
    for (e1 in 1:length(etas_list_1)) {
      ...
      for (e2 in 1:length(etas_list_2)) {
        ...
        ###### SHOW LOOP STEP ######
        print(paste("DATASET FILE - ", dataset_file_name))
        ...
        for(i in 1:folds_num) {
          ...
          # TESTING VALUES FOLDS
          Fold_testing <- c()
          for (indian_env in india_envs) {
            Fold_testing <- c(Fold_testing, folds[[i]][[indian_env]]$testing)
          }
...
write.csv(Predictions_Final, paste(results_dir, paste("predictions_", dataset_file_name, "_Test.csv", sep = "")))
write.csv(Tb_A  , paste(results_dir, paste("summary_ALL_", dataset_file_name, "_Test.csv", sep = "")))
write.csv(Sumary_all , paste(results_dir, paste("summary_", dataset_file_name, "_Test.csv", sep = "")))

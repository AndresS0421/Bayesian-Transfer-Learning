...
generate_multivariate_folds <- function(lines,
  ...
  for (fold_num in seq(folds_num)) {
    ...
    folds[[fold_num]] <- list(num = fold_num)

    if (type == "ibd") {
      print(paste(unique_lines_num, unique_envs_num, k))
...
folds_by_model <- function(model, Pheno, folds_num, testing_proportion) {
  model <- tolower(model)

  training_proportion <- 1 - testing_proportion
  training_lines_num <- ceiling(SKM::lunique(Pheno$Line) * training_proportion)

  if (model == "m1") {
    return(cv_env_random_line(
      lines = Pheno$Line,
      envs = Pheno$Env,
      folds_num = folds_num,
      training_lines_num = training_lines_num,
    ...
    return(cv_env_partial_random_line(
      lines = Pheno$Line,
      envs = Pheno$Env,
      folds_num = folds_num,
      training_lines_num = training_lines_num,

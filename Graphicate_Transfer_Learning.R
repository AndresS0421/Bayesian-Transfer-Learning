...
results_avg <- results_dataset %>%
  group_by(Testing_Proportion) %>%
  summarise(
    across(
      .cols = starts_with("COR_") | starts_with("MSE_") | starts_with("NRMSE_") | starts_with("PM_"),
      .fns = ~ mean(.x, na.rm = TRUE),
      .names = "{.col}"  # Naming the output columns
    ),
...
results_avg_long <- data.frame(
  ...
  COR_SD = c(results_avg$COR_BayNI_SD, results_avg$COR_Trans_SD),
  PM_10 = c(results_avg$PM_Conv_Ave_10, results_avg$PM_Transf_Ave_10),
  PM_10_SD = c(results_avg$PM_Conv_SD_10, results_avg$PM_Transf_SD_10),
  PM_20 = c(results_avg$PM_Conv_Ave_20, results_avg$PM_Transf_Ave_20),
  PM_20_SD = c(results_avg$PM_Conv_SD_20, results_avg$PM_Transf_SD_20),
  PM_30 = c(results_avg$PM_Conv_Ave_30, results_avg$PM_Transf_Ave_30),
  PM_30_SD = c(results_avg$PM_Conv_SD_30, results_avg$PM_Transf_SD_30)
)
...
for (data_set in data_sets) {
  data_set = data_set
  results_to_graph <- list(droplevels(results_trait_long[results_trait_long$Dataset == data_set, ]), 
                           droplevels(results_dataset_long[results_dataset_long$Dataset == data_set, ])
                      )
...
print("AVERAGE")
### Create Dir -------------------------------------------------------------------
dir.create(file.path(plots_dir, "AVERAGE"))  
### Graph ALL Data ---------------------------------------------------------------
results_long_colnames <- colnames(results_avg_long)
for (col_data_index in seq(5, length(results_long_colnames), by = 2)) {
  col_data <- results_long_colnames[col_data_index]
  col_data_sd <- results_long_colnames[col_data_index + 1]
  cat("Column: ", col_data, "\n")
  ...
  for (results_index in 1:length(results_to_graph)) {
    ...
    ### Create Graph File ----------------------------------------------------------
    save_plot(Plot, file = file.path(plots_dir, "AVERAGE", paste0(col_data, "_AVG", ".png")))
  }
}